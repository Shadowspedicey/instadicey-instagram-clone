...
const PostCard = ({post}) =>
{
	...
	{
		// TODO: Get comments as array and set them
	};
	useEffect(() =>
	{
		const getData = async () =>
	...
	if (!post || !comments) return <div className="post-card loading outlined"><div className="container"><img src={Loading} alt="loading" style={{height: "25%", position: "absolute", inset: 0, margin: "auto"}}></img></div></div>;
	else return(
				...
				<Link to={`/p/${post.id}`}>
					{ isHovered &&
						<div className="hovered">
							<span><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M12 4.248c-3.148-5.402-12-3.825-12 2.944 0 4.661 5.571 9.427 12 15.808 6.43-6.381 12-11.147 12-15.808 0-6.792-8.875-8.306-12-2.944z"></path></svg> {post.likes.length}</span>
